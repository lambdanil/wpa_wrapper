...
os.system("bash wr_status")
statefile = open("state.txt", "r")
statel = statefile.readline()
if 'UP' in statel:
    state = 'on'
else:
    state = 'off'
...
while option == '4':
    os.system("bash clear_conf")
    option = 'exit'

if option == '0':
    os.system("bash clear_temp")
    clear()
